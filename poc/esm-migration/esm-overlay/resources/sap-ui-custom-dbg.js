...
(function(__global) {
	...
	// ---- ESM Module Support (experimental) --------------------------------------------------------

	/**
	 * Registry of modules that have been converted to ESM format.
	 * Maps UI5 module names (e.g. "sap/base/strings/camelize.js") to their ESM file paths
	 * (relative URLs that can be passed to dynamic import()).
	 *
	 * When a module is registered here, the loader will use import() instead of script tags
	 * to load it, and the module's default export will be used as the module value.
	 *
	...
	/**
	 * Register a module as ESM-capable. When the loader needs to load this module,
	 * it will use dynamic import() instead of a script tag.
	 *
	 * @param {string} sModuleName UI5 module name WITHOUT .js extension (e.g. "sap/base/strings/camelize")
	 * @param {string} [sESMPath] Path to the ESM file. If omitted, derived from the module name + "-dbg.js"
	 */
	function registerESMModule(sModuleName, sESMPath) {
		const sResourceName = sModuleName + ".js";
		if (!sESMPath) {
			// Default: same path as the debug variant, with -dbg.js suffix
	...
	/**
	 * Check if a module has been registered as an ESM module.
	 *
	 * @param {string} sResourceName Module resource name WITH .js extension
	 * @returns {boolean}
	 */
	...
	/**
	 * Load an ESM module using dynamic import() and resolve it into the ui5loader registry.
	 *
	 * @param {object} oModule The ui5loader Module object
	 * @param {string} sResourceName The resource name (e.g. "sap/base/strings/camelize.js")
	 */
	function loadESM(oModule, sResourceName) {
		...
		if ( log.isLoggable() ) {
			log.debug(sLogPrefix + "loading '" + sResourceName + "' from '" + sFullUrl + "' (using ESM import())");
		}
		...
		import(sFullUrl).then(function(esmModule) {
			...
			log.error("failed to load ESM module: " + sResourceName + " - " + err.message);
			oModule.failWith("failed to load ESM module {id}", err);
		});
	}

	// ---- end ESM Module Support -------------------------------------------------------------------
	...
	function requireModule(oRequestingModule, sModuleName, bAsync, bSkipShimDeps, bSkipBundle) {
		...
		// --- ESM Module Interception (experimental) ---
		// If this module is registered as ESM, use dynamic import() instead of script tags.
		// This path only works for async loading (which is the standard in modern UI5).
	...
	const privateAPI = {
		 ...
		 * it will use dynamic import() instead of a script tag.
		 *
		 * @param {string} sModuleName Module name WITHOUT .js extension
		 * @param {string} [sESMPath] Path to the ESM file. Defaults to moduleName + "-dbg.js"
		 */
		registerESMModule,
		/**
		 * Check if a module is registered as ESM.
		 * @param {string} sResourceName Resource name WITH .js extension
		 * @returns {boolean}
		 */
		isESMModule,
		/**
		 * Get the ESM module registry (read-only copy).
		 * @returns {Map<string, string>}
		 */
		getESMModules() {
			return new Map(mESMModules);
		}
	};
	...
	/**
	 * Register a module as ESM-capable. When the loader needs to load this module,
	 * it will use dynamic import() instead of a script tag / XHR.
	 *
	 * @param {string} sModuleName Module name WITHOUT .js extension (e.g. "sap/base/strings/camelize")
	 * @param {string} [sESMPath] Path to the ESM file. Defaults to moduleName + "-dbg.js"
	 * @private
	 * @experimental
	 * @ui5-global-only
	 * @name sap.ui.registerESMModule
	 */
