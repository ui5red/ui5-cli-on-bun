...
- `ui5.v1.shopping.cart` has the implemented bridge-free source/build path and the corrected app-root serving commands, and the latest source-root and release smoke now render the `cart---app` root view under a mounted `sapUiComponentContainer`.
- `ui5.v2.shopping.cart` now reaches the same mounted source-native runtime state in both source-root and release mode, with the latest smoke showing the same `cart---app` root view DOM and no surfaced `#esm-errors`.
